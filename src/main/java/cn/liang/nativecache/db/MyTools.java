...
public class MyTools {
    ...
    public static <T> List<T> groupList(String split, List<T> list, final String... objs) {
        Map<String, T> result = new HashMap<String, T>();
        try {
            String key = "";
            for (T ol : list) {
                ...
                if (result.containsKey(key)) {
                    ...
                    result.put(key, addTwoSimpleValue(result.get(key), value));
                } else {
                    result.put(key, ol);
                }
            }
    ...
    private static <T> T addTwoSimpleValue(T value1, T value2) throws IllegalAccessException {
        Class clazz = value1.getClass();
        Field[] fields = clazz.getDeclaredFields();
        for (Field field : fields) {
            addTwoField(field, value1, value2);
        }
        return value1;
    }

    private static <T> void addTwoField(Field field, T value1, T value2) throws IllegalAccessException {
        Object v1 = ReflectUtils.getValue(value1, field);
        Object v2 = ReflectUtils.getValue(value2, field);
        if (v1 == null && v2 != null) {
            ReflectUtils.setValue(value1, field, v2);
        } else if (v1 != null && v2 == null) {
            ReflectUtils.setValue(value1, field, v1);
        }
        if (v1 != null && v2 != null) {
            Object add = v1;
            if (v1 instanceof Integer) {
                add = Integer.parseInt(v1.toString()) + Integer.parseInt(v2.toString());
                ReflectUtils.setValue(value1, field, add);
            } else if (v1 instanceof Long) {
                add = Long.parseLong(v1.toString()) + Long.parseLong(v2.toString());
                ReflectUtils.setValue(value1, field, add);
            } else if (v1 instanceof Double) {
                add = Double.parseDouble(v1.toString()) + Double.parseDouble(v2.toString());
                ReflectUtils.setValue(value1, field, add);
            } else if (v1 instanceof Float) {
                add = Float.parseFloat(v1.toString()) + Float.parseFloat(v2.toString());
                ReflectUtils.setValue(value1, field, add);
            } else if (v1 instanceof Short) {
                add = Short.parseShort(v1.toString()) + Short.parseShort(v2.toString());
                ReflectUtils.setValue(value1, field, add);
            }
        }
    }
